...
class FindLatestGradleReleasesAction :
  GradleUtilityAction<LatestGradleReleases>(
  ...
  companion object {
    ...
    @TestOnly
    val GRADLE_RELEASE_NAME_VERSION_REGEX =
      Regex("^(?<major>\\d+)(?:.(?<minor>\\d+))?(?:.(?<patch>\\d+))?(?:\\s+(?<preRelease>.+))?\$")
  }
  ...
  override fun runAction(executionContext: ExecutionContext, progressIndicator: ProgressIndicator) {
    ...
    result(
      LatestGradleReleases(
        latestProductiveGradleReleases.values.toList(),
  ...
  fun requestLatestGradleGitHubReleases(): List<GradleGitHubRelease> {
    HttpClients.custom().setSSLContext(CertificateManager.getInstance().sslContext).build().use {
      httpclient ->
      val releasesGet =
        HttpGet("https://api.github.com/repos/gradle/gradle/releases?per_page=100").apply {
      ...
      httpclient.execute(releasesGet).use { response ->
        if (response.statusLine.statusCode != 200) {
          throw GradleUtilityActionFailedException(
            "GitHub API returned the unexpected status code ${response.statusLine.statusCode} " +
        ...
        val releases =
          objectMapper.readValue(responseEntity.content, Array<GradleGitHubRelease>::class.java)
