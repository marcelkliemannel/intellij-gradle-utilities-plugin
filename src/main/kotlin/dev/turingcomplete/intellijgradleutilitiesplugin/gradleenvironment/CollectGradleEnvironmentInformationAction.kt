...
class CollectGradleEnvironmentInformationAction :
  GradleWrapperAction<GradleEnvironment>(
  ...
  override fun runAction(executionContext: ExecutionContext, progressIndicator: ProgressIndicator) {
    ...
    val projectDir =
      project.guessProjectDir()
        ?: throw GradleUtilityActionFailedException("Failed to determine project directory.")
    ...
    val gradleSystem =
      collectSystemGradleInformation(executionContext.project, progressIndicator, handleError)
    ...
    val projectProperties =
      readProperties(
        ...
        projectDir.toNioPath().resolve("gradle.properties"),
        progressIndicator,
        handleError,
      )
    val userProperties =
      readProperties(
        ...
        gradleUserHomeDir.resolve("gradle.properties"),
        progressIndicator,
        handleError,
      )
  ...
  private fun collectSystemGradleInformation(
    project: Project?,
    progressIndicator: ProgressIndicator,
    handleError: (String, Exception) -> Unit,
  ): GradleEnvironment.SystemGradle? {
    ...
    try {
      ...
      val systemGradleVersion: String? =
        GradleUtils.findSystemGradleExecutable(project)?.let { systemGradleExecutable ->
  ...
  private fun collectGradleWrapperInformation(
    projectDir: VirtualFile,
    progressIndicator: ProgressIndicator,
    handleError: (String, Exception) -> Unit,
  ): GradleEnvironment.GradleWrapper? {
    progressIndicator.checkCanceled()

    try {
      progressIndicator.text = "Finding Gradle wrapper JAR..."
      val gradleWrapperJarChecksum =
        findGradleWrapperJar(projectDir, progressIndicator)?.let { gradleWrapperJar ->
      ...
      val gradleWrapperVersion =
        findGradlewExecutable(projectDir, progressIndicator)?.let { gradlewExecutable ->
      ...
      val gradleWrapperProperties =
        readGradleWrapperProperties(projectDir, progressIndicator, handleError)

      val checksumVerificationConfigured =
        GradleUtils.isChecksumVerificationConfigured(gradleWrapperProperties)

      return GradleEnvironment.GradleWrapper(
  ...
  private fun calculateGradleWrapperJarChecksum(
    progressIndicator: ProgressIndicator,
    gradleWrapperJar: VirtualFile,
  ): String {
    progressIndicator.checkCanceled()
