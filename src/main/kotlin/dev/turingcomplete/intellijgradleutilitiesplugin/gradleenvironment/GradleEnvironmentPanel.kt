...
class GradleEnvironmentPanel(private val gradleEnvironment: GradleEnvironment) :
  ...
  init {
    ...
    add(
      JBLabel("Project Gradle properties:"),
      bag.nextLine().next().overrideTopInset(UIUtil.DEFAULT_HGAP),
    )
    add(
      createPropertiesTable(
        gradleEnvironment.projectProperties,
        listOf("Key", "Value"),
        "Project does not have any Gradle properties set",
      ),
      bag.nextLine().next().weighty(0.4).fillCell().overrideTopInset(2),
    )

    add(
      JBLabel("User Gradle properties:"),
      bag.nextLine().next().overrideTopInset(UIUtil.LARGE_VGAP),
    )
    add(
      createPropertiesTable(
        gradleEnvironment.userProperties,
        listOf("Key", "Value"),
        "User does not have any Gradle properties set",
      ),
      bag.nextLine().next().weighty(0.4).fillCell().overrideTopInset(2),
    )

    add(
      JBLabel("Gradle related environment variables:"),
      bag.nextLine().next().overrideTopInset(UIUtil.LARGE_VGAP),
    )
    add(
      createPropertiesTable(
        gradleEnvironment.environmentVariables,
        listOf("Name", "Value"),
        "No Gradle related environment variables",
  ...
  private fun createPathsComponent(): JComponent {
    return JPanel(GridBagLayout()).apply {
      ...
      add(
        createDirHyperlink(gradleEnvironment.gradleUserHomeDir),
        bag.next().weightx(1.0).fillCellHorizontally().overrideLeftInset(4),
      )
    }
  }
  ...
  private fun createDirHyperlink(dir: Path): HyperlinkLabel {
    return HyperlinkLabel(dir.toString()).apply {
      ...
      addHyperlinkListener { BrowserUtil.browse(dir) }
    }
  }
  ...
  private fun createSystemGradleComponent(): JComponent {
    return JPanel(GridBagLayout()).apply {
      ...
      if (gradleSystem == null) {
        ...
        add(
          createDirHyperlink(gradleSystem.gradleHomeDir),
          bag.next().weightx(1.0).fillCellHorizontally().overrideLeftInset(4),
        )

        add(JBLabel("Version:"), bag.nextLine().next().overrideTopInset(UIUtil.DEFAULT_VGAP))
        add(
          JBLabel(gradleSystem.version ?: "Unknown").copyable(),
          bag
            .next()
            .weightx(1.0)
  ...
  private fun createGradleWrapperComponent(): JComponent {
    return JPanel(GridBagLayout()).apply {
      ...
      if (gradlewWrapper == null) {
        add(
          JBLabel("Couldn't find Gradle wrapper JAR and/or executable in project."),
          bag.nextLine().next(),
        )
      } else {
        add(JBLabel("Version:"), bag.nextLine().next())
        add(
          JBLabel(gradlewWrapper.version ?: "Unknown").copyable(),
          bag.next().weightx(1.0).fillCellHorizontally().coverLine().overrideLeftInset(4),
        )

        add(JBLabel("Checksum:"), bag.nextLine().next().overrideTopInset(UIUtil.DEFAULT_VGAP))
        add(
          JBLabel(gradlewWrapper.checksum ?: "Unknown").copyable(),
          bag
            .next()
            .weightx(1.0)
            .fillCellHorizontally()
        ...
        if (gradlewWrapper.checksumVerificationConfigured) {
          add(
            ...
            bag.nextLine().next().next().overrideTopInset(UIUtil.DEFAULT_VGAP),
          )
          add(
            UiUtils.createLink(
              "Learn more",
            ...
            bag.next().overrideLeftInset(2).overrideTopInset(UIUtil.DEFAULT_VGAP),
          )
        } else {
          add(
            JBLabel(
              "Checksum verification not configured.",
        ...
        add(
          JBLabel("Gradle wrapper properties:"),
          bag.nextLine().next().coverLine().overrideTopInset(UIUtil.DEFAULT_VGAP),
        )
        add(
          createPropertiesTable(
            gradlewWrapper.wrapperProperties,
            listOf("Key", "Value"),
            "No Gradle wrapper proeprties",
          ),
          bag.nextLine().next().coverLine().weighty(0.4).fillCell().overrideTopInset(2),
        )
      }
    }
  }

  private fun createPropertiesTable(
    data: List<Pair<String, String?>>,
    columnNames: List<String>,
    noDataText: String,
    singleDataName: String = "Property",
    pluralDataName: String = "Properties",
  ): JComponent {
  ...
  private class PropertiesTableModel(
    ...
    override fun getValueAt(row: Int, column: Int) =
      data[row].let { if (column == 0) it.first else it.second }
  }

  // -- Inner Type ---------------------------------------------------------- //

  private class PropertiesTable(
    private val tableModel: PropertiesTableModel,
    noDataText: String,
    singleDataName: String,
    pluralDataName: String,
  ) : JBTable(tableModel), DataProvider {

    companion object {
      val SELECTED_PROPERTIES_DATA_KEY =
        DataKey.create<List<Pair<String, String?>>>("selectedProperties")
    }
    ...
    init {
      ...
      val contextMenuActions =
        DefaultActionGroup(CopyAction(singleDataName, pluralDataName), ExpandValue(singleDataName))
      addMouseListener(
  ...
  private class CopyAction(private val singleDataName: String, private val pluralDataName: String) :
    ...
    override fun update(e: AnActionEvent) {
      val selectedProperties =
        PropertiesTable.SELECTED_PROPERTIES_DATA_KEY.getData(e.dataContext)
      ...
      e.presentation.text =
        if (nonNullSelectedProperties.size > 1) {
  ...
  private class ExpandValue(private val singleDataName: String) :
    ...
    override fun update(e: AnActionEvent) {
      ...
      e.presentation.isVisible =
        selectedProperties.size == 1 && selectedProperties[0].second != null
    }
