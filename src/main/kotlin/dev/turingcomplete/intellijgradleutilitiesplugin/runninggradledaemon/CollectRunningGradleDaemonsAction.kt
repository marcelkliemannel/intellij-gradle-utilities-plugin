...
class CollectRunningGradleDaemonsAction :
  GradleUtilityAction<List<GradleDaemon>>(
    "Collect Running Gradle Daemons",
  ...
  companion object {
    ...
    val DETERMINE_DAEMON_STATUS: DataKey<Boolean> =
      DataKey.create("Gradle.Utilities.Plugin.CollectRunningGradleDaemonsAction.FindDaemonStatus")
  }
  ...
  override fun runAction(executionContext: ExecutionContext, progressIndicator: ProgressIndicator) {
    ...
    val gradleDaemonProcesses: Map<Long, ProcessHandle> =
      ProcessHandle.allProcesses()
        ...
        .filter { process ->
          return@filter try {
            ...
            LOGGER.warn(
              "Failed to read command line of process with PID ${process.pid()} to check if it is a Gradle daemon process.",
              e,
            )
            false
          }
    ...
    if (DETERMINE_DAEMON_STATUS.getData(executionContext.dataContext) == true) {
      ...
      workingDirToGradleExecutable.forEach { (workingDir, gradleExecutable) ->
        progressIndicator.checkCanceled()
        GradleUtils.determineGradleDaemonStatus(workingDir, gradleExecutable).forEach {
          (pid, status) ->
    ...
    gradleDaemonProcesses
      .filter { !gradleDaemons.containsKey(it.key) }
      .forEach { gradleDaemons[it.key] = GradleDaemon(it.value, null) }
